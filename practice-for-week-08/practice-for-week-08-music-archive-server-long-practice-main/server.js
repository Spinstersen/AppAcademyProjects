...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === "POST" && req.url === "/artists") {
      ...
      const artistObj = JSON.parse(reqBody);
      artistInfo = { artistId: newId, name: artistObj.name };
    ...
    if (
      ...
      if (urlParts.lenght === 3) {
        const artistId = urlParts[2];
        const artistObj = JSON.parse(reqBody);
        artists[artistId].name = artistObj.name;
        let date = Date();
        artists[artistId].updatedAt = date;
